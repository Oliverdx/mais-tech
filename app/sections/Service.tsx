    ...
    <div className="md:max-w-[1000px] md:m-auto">
      ...
      <div className="w-full">
        ...
        <div className="pb-2 flex items-center mt-2 mb-4">
          ...
          />
          <h3 className={`text-[11px] w-auto md:text-[12px] uppercase font-bold h-[35px] flex items-center px-10 md:w-[300px] m-[-17px] bg-${isPrimary ? 'main_red' : 'white'}`}
            style={{ color: isPrimary ? 'white' : 'var(--main-red)' }}>
            Metodologia Mais Tech de Riscos
          </h3>
        </div>
      </div>
        ...
        <div className="flex flex-col items-center m-auto md:items-end">
          <div className="flex justify-center items-center gap-2 mb-8 w-[300px]">
            ...
            <div>
              ...
              <Link href={`${NUMBER_WHATSAPP} e gostaria de mais informações`} className={`uppercase font-bold text-xl ${isPrimary ? 'text-main_red' : 'text-white'}`}>
                {PHONE_NUMBER}
              </Link>
            </div>
          </div>
          <Link href={`${NUMBER_WHATSAPP} e gostaria de mais informações`} className="flex flex-col items-center gap-4 w-[160px]">
            <Image
            ...
            />
            <span className={`font-bold text-[14px] text-center rounded p-2 uppercase ${isPrimary ? 'bg-main_red text-white' : 'bg-white text-main_red'}`}>
              Leia o QRCode ou clique e contrate agora!
            </span>
          </Link>
        </div>
      )}
      ...
      {bgImage !== "" && (
        <div className="flex align-end relative w-[280px] h-[200px] mt-10 m-auto md:w-[700px] md:h-[530px] md:mt-[-30px]">
          <Image
            src={bgImage}
